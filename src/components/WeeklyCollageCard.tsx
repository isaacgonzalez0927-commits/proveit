...
export function WeeklyCollageCard({ collage, compact = false }: WeeklyCollageCardProps) {
    ...
    <article className="overflow-hidden rounded-2xl glass-card">
      <div className="flex items-center justify-between gap-3 border-b border-slate-200/70 px-4 py-3 dark:border-slate-700/70">
        <div>
          ...
          <h3 className="mt-0.5 text-sm font-semibold text-slate-900 dark:text-white">{collage.label}</h3>
        </div>
      ...
      {!compact && collage.photos.length > 0 && (
