...
import { loadImage, roundRect } from "@/lib/shareImage";
...
export async function drawGardenSnapshotRow(
  ...
  options?: { label?: string; lightPanel?: boolean }
): Promise<void> {
  const visible = plants.slice(0, 8);
  if (visible.length === 0) return;

  const lightPanel = options?.lightPanel ?? true;
  roundRect(ctx, box.x, box.y, box.width, box.height, 24);
  ctx.fillStyle = lightPanel ? "rgba(255,255,255,0.22)" : "rgba(255,255,255,0.12)";
  ctx.fill();
  ctx.strokeStyle = lightPanel ? "rgba(255,255,255,0.38)" : "rgba(255,255,255,0.28)";
  ctx.lineWidth = 2;
  ctx.stroke();

  if (options?.label) {
    ctx.fillStyle = lightPanel ? "rgba(255,255,255,0.92)" : "rgba(255,255,255,0.85)";
    ctx.font = "600 24px system-ui, -apple-system, sans-serif";
    ctx.textAlign = "left";
    ctx.fillText(options.label, box.x + 24, box.y + 36);
  }

  const padX = 20;
  const padBottom = 18;
  const labelSpace = options?.label ? 44 : 12;
  const innerW = box.width - padX * 2;
  const innerH = box.height - labelSpace - padBottom;
  const slotW = innerW / visible.length;
  const maxPlantH = innerH;
  const maxPlantW = Math.min(118, slotW * 0.92);
  ...
  for (let i = 0; i < visible.length; i += 1) {
    ...
    const y = box.y + box.height - padBottom - dh;
